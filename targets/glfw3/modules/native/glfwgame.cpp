...
//	case GLFW_KEY_RCTRL:return VKEY_RCONTROL;
	case GLFW_KEY_LEFT_ALT:return VKEY_LEFT_ALT;
	case GLFW_KEY_RIGHT_ALT:return VKEY_RIGHT_ALT;
...
void BBGlfwGame::Run(){
	...
	while( !glfwWindowShouldClose( _window ) ){
		...
		if( fabs(_nextUpdate) > DBL_EPSILON ){
		...
		for( ;i<4;++i ){
			...
			UpdateGame();
			if( fabs(_nextUpdate) < DBL_EPSILON ) break;
