...
function gxtkGraphics(){
	...
	if (typeof CFG_MOJO_IMAGE_FILTERING_ENABLED !== "undefined" && CFG_MOJO_IMAGE_FILTERING_ENABLED === "0"){
		...
		gc['mozImageSmoothingEnabled'] = false;
		gc['oImageSmoothingEnabled'] = false;
		gc['webkitImageSmoothingEnabled'] = false;
		gc['msImageSmoothingEnabled'] = false;
	}
}
...
if( CFG_HTML5_WEBAUDIO_ENABLED=="1") {
	window.AudioContext = window.AudioContext || window.webkitAudioContext;
}

if( CFG_HTML5_WEBAUDIO_ENABLED=="1" && AudioContext && AudioContext.prototype.createGain){
...
gxtkSample.prototype.Load=function( path ){
	...
	BBHtml5Game.Html5Game().LoadAudioData(BBGame.Game().PathToUrl( path ), (function( data ) {
		if ( data ) {
			this.waBuffer=data;
			this.state=1;
		} else {
...
var gxtkAudio=function(){
	...
	if( !window['wa'] ){
		...
		var canvas = this.game.GetCanvas();
		var oldTouch = canvas.ontouchend;

		canvas.ontouchend = function (e) {
			canvas.ontouchend = oldTouch;
			var wa = window['wa'];
...
gxtkAudio.prototype.LoadSample=function( path ){
	...
	var url = this.game.PathToUrl( path );
	var audio = null;

	if (window['PRELOADER_CACHE'] && window['PRELOADER_CACHE']['AUDIO'][url]) {
		audio = window['PRELOADER_CACHE']['AUDIO'][url];
	} else {
		audio = new Audio( url );
	}
