...
Function EndRender()
#If MOJO_USE_HARMONY
	If (renderDevice) renderDevice.EndRender()
#End		
	renderDevice=Null
End
...
Function PopMatrix()
	...
	Local ms := context.matrixStack
	SetMatrix ms[sp+0], ms[sp+1], ms[sp+2], ms[sp+3], ms[sp+4], ms[sp+5]
	context.matrixSp=sp
End
...
Function Rotate( angle:Float )
	Local ca := Cos(angle), sa := Sin(angle)
	Transform ca,-sa,sa,ca,0,0
End Function
...
Function DrawImage( image:Image, x:Float, y:Float, frame = 0 )
...
Function DrawImage( image:Image, x:Float, y:Float, rotation:Float, scaleX:Float, scaleY:Float, frame = 0 )
...
Function DrawImageRect( image:Image, x:Float, y:Float, srcX, srcY, srcWidth, srcHeight, frame = 0 )
