...
struct BBDisplayMode : public Object{
	...
	int depth;
	int hertz;
	int options; //becase gc_object has flags
	BBDisplayMode( int width=0,int height=0,int depth=0,int hertz=0,int flags=0 ):width(width),height(height),depth(depth),hertz(hertz),options(flags){}
};
