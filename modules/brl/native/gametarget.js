...
BBGame.prototype.SaveState=function( state ){
	window['localStorage'].setItem( "monkeystate@"+document.URL,state );	//key can't start with dot in Chrome!
	return 1;
}

BBGame.prototype.LoadState=function(){
	var state=window['localStorage'].getItem( "monkeystate@"+document.URL );
	if( state ) return state;
	return "";
}

BBGame.prototype.LoadString=function( path ){
	var path =  this.PathToUrl( path );
	if (window['PRELOADER_CACHE'] && window['PRELOADER_CACHE']['TEXT'][path]) {
		return window['PRELOADER_CACHE']['TEXT'][path];
	}
...
BBGame.prototype.IsCrossOriginUrl=function( url ){
	return false;
}
...
BBGame.prototype.LoadAudioData=function( path,listener ){
	listener(null);
}
...
BBGame.prototype.StartGame=function(){
	...
	if( this._startPreformed ) return;
	this._startPreformed = true;
