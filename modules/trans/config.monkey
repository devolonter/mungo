...
Class ConfigScope Extends ScopeDecl
	...
	Field overridableVars:= New StringSet
...
Function SetConfigVar(key:String, val:String, overridable:Bool = False)
	SetConfigVar(key, val, Type.stringType, overridable)
End

Function SetConfigVar(key:String, val:String, type:Type, overridable:Bool = False)
	...
	If (overridable) Then
		_cfgScope.overridableVars.Insert(key)
	Else
		_cfgScope.overridableVars.Remove(key)
	End If
End
...
Function IsOverridableConfigVar:Bool(key:String)
	Return Not _cfgScope.vars.Contains(key) Or _cfgScope.overridableVars.Contains(key)
End
