...
Function PreProcess:String(path:String, mdecl:ModuleDecl=Null, overridable:Bool = False)
				...
				If ty=TOKE_IDENT
					...
					Case "=","+="
						...
						Case "="
							Local expr:=EvalExpr( toker )
							Local val:=expr.Eval()
							If IsOverridableConfigVar(toke)
								If StringType( expr.exprType )  val=EvalConfigTags( val )
								SetConfigVar toke, val, expr.exprType, overridable
							Endif
						Case "+="
							...
							SetConfigVar toke,var+val, overridable
						End
