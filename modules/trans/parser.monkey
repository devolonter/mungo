...
'				If Not static And expr Return New StmtExpr( New ExprStmt( expr ),cexpr ).Semant()
	...
	Method SemantFunc:Expr( args:Expr[] )
		...
		Local cls:=scope.ClassScope()
		Local fdecl:FuncDecl
		
		If cls fdecl = cls.FindFuncDecl(ident,args,,True)				
		If (Not fdecl) fdecl=scope.FindFuncDecl( ident,args )
	...
	Method ParseFuncDecl:FuncDecl( attrs )
		...
		Repeat		
			...
			Else If CParse( "abstract" )
				...
				attrs|=DECL_ABSTRACT
			Else If CParse( "inline" )
				If attrs & DECL_ABSTRACT Err "Methods cannot be both abstract and inline."
				If attrs & FUNC_PROPERTY Err "Duplicate method attribute."
				attrs|=FUNC_INLINE|DECL_FINAL
...
Function ParseSource( source:String, app:AppDecl, mdecl:ModuleDecl, defattrs = 0 )
	
	If (Parser.TCC And Parser.TCC.opt_scriptspath)
		source = Parser.TCC.ExecuteScripts("pre_parse", source)
	End
