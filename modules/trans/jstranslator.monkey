...
Class JsTranslator Extends CTranslator
	...
	Method TransLocalDecl$( munged$,init:Expr )
		If annotationsEnabled
			Emit "/**"
			Emit "* @type {" + GetClosureType(init.exprType) + "}"
			Emit "*/"
		End If
...
'				If ENV_CONFIG="debug" t_lhs="dbg_object"+Bra(t_lhs)
	...
	Method TransNewObjectExpr:String( expr:NewObjectExpr )
		Local t:String = "new "+expr.classDecl.munged
		If expr.ctor
	...
	Method TransNewArrayExpr$( expr:NewArrayExpr )
		...
		If IntType( ty ) Return "new_int_array("+texpr+")"
		If FloatType( ty ) Return "new_float_array("+texpr+")"
	...
	Method TransSliceExpr$( expr:SliceExpr )
		...
		Local call:String = "slice"
		If ArrayType( expr.exprType ) And GetConfigVar("JS_USE_TYPED_ARRAYS") = "1"
			If IntType(ArrayType(expr.exprType).elemType) Or FloatType(ArrayType(expr.exprType).elemType) call = "subarray"
		End If
		
		Return t_expr+"."+call+"("+t_args+")"
	End
	...
	Method TransArrayExpr$( expr:ArrayExpr )
		...
		If GetConfigVar("JS_USE_TYPED_ARRAYS") = "1"
			If IntType(ArrayType(expr.exprType).elemType) Return "new Int32Array(["+t+"])"
			If FloatType(ArrayType(expr.exprType).elemType) Return "new Float32Array(["+t+"])"		
		End If
	...
	Method TransIntrinsicExpr$( decl:Decl,expr:Expr,args:Expr[] )
		...
		Case "resize"
			...
			If IntType( ty ) Return "resize_int_array"+Bra( texpr+","+arg0 )
			If FloatType( ty ) Return "resize_float_array"+Bra( texpr+","+arg0 )
	...
	Method EmitFuncDecl( decl:FuncDecl )
		...
		If annotationsEnabled
			Local this:Bool = decl.IsCtor() Or decl.IsMethod() Or decl.IsProperty()
					
			If (decl.argDecls Or Not VoidType(decl.retType) Or this Or decl.overrides Or decl.IsStatic())
				Emit "/**"
				If (decl.IsFinal()) Then
					Emit "* @final"
				End If
				If (decl.overrides) Then
					Emit "* @override"
				End If
				If (this) Then
					Emit "* @this {"+decl.scope.munged+"}"
				End If
				If (decl.IsStatic())
					Emit "* @static"
				EndIf
				For Local arg:=EachIn decl.argDecls
					Emit "* @param {"+GetClosureType(arg.type)+"}"
				Next
				If (Not VoidType(decl.retType)) Then
					Emit "* @return {"+GetClosureType(decl.retType)+"}"
				End If
				Emit "*/"
			End If
		End If
	...
	Method EmitClassDecl( classDecl:ClassDecl )
		...
		If annotationsEnabled
			Emit "/**"
			Emit "* @constructor"
			Emit "* @extends {"+superid+"}"
			
			If (classDecl.IsFinal()) Then
				Emit "* @final"
			End If
		...
		For Local decl:=Eachin classDecl.Semanted
			...
			If fdecl
				If annotationsEnabled
					Emit "/**"
					Emit "* @type {" + GetClosureType(fdecl.type) + "}"
					Emit "*/"
				End If
			
				Emit "this." + fdecl.munged + "=" + fdecl.init.Trans() + ";"
			End
		Next
		...
		For Local decl:=Eachin classDecl.Semanted
			...
			If gdecl
				If annotationsEnabled
					Emit "/**"
					Emit "* @type {"+GetClosureType(gdecl.type)+"}"
					Emit "* @static"
					Emit "*/"
				End If
	...
	Method GetClosureType:String(type:Type)
		If (BoolType(type)) Then
			Return "boolean"
		Else If (NumericType(type)) Then
			Return "number"
		Else If (StringType(type)) Then
			Return "string"
		Else If (ArrayType(type)) Then
			Return "Array<"+GetClosureType(ArrayType(type).elemType)+">"
		Else If (ObjectType(type)) Then
			Return ObjectType(type).classDecl.munged
		Else
			Return "?"
		End If
	End Method
	...
	Method TransApp$( app:AppDecl )
		annotationsEnabled = GetConfigVar("HTML5_OPTIMIZE_OUTPUT") = "1" And GetConfigVar("HTML5_OPTIMIZATION_LEVEL") = "advanced"
