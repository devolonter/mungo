...
Public
	...
	Method FindFuncDecl:FuncDecl( ident$,argExprs:Expr[],explicit=False,softFail?=False )
...
Class ClassDecl Extends ScopeDecl
	...
	Method FindFuncDecl:FuncDecl( ident$,args:Expr[],explicit=False,softFail?=False )
	
		If Not IsInterface()
			Local result:=FindFuncDecl2( ident,args,explicit,superClass<>Null )
			If result Return result
			
			Local sclass:=superClass
			While sclass
				result=sclass.FindFuncDecl2( ident,args,explicit,sclass.superClass<>Null )
				
				If result Then
					If result.IsCtor
						result = FuncDecl(result.Copy())
						result.retType = objectType
						
						If Not (result.attrs & FUNC_CALLSCTOR)
							Local ctor:=sclass.FindFuncDecl2( "new",[],False )
						
							If ctor
								Local expr:=New InvokeSuperExpr( "new",[] )
								expr.funcDecl = ctor
								result.stmts.AddFirst New ExprStmt( expr )
							Endif
						Endif
	...
	Method FindFuncDecl2:FuncDecl( ident$,args:Expr[],explicit,softFail?=False )
		Return Super.FindFuncDecl( ident,args,explicit,softFail )
	End
