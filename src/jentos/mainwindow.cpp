...
QWidget *MainWindow::openFile( const QString &cpath,bool addToRecent ){
    ...
    /*if( isUrl( cpath ) ){
        if(_isShowHelpInDock) {
            _ui->webView->page()->setLinkDelegationPolicy( QWebEnginePage:: );
            _ui->webView->setUrl( cpath );
            _ui->docsDockWidget->setVisible(true);
            return 0;
        }
    ...
    QString fileinvalids[4] = {"png","ico","jpg","gif"};
...
bool MainWindow::isValidMonkeyPath( const QString &path, QString &trans ){
    ...
    s = QDir::currentPath() + "/bin/" + trans;
    r = QFile::exists(s);
    if(r) {
        _monkeyPath = QDir::currentPath();
        _transPath = trans;
        return true;
    }
    return false;
...
void MainWindow::readSettings(){
    ...
    QString tr;
    if ( !isValidMonkeyPath(_monkeyPath, tr) ) {
...
void MainWindow::onProjectMenu( const QPoint &pos ){
    ...
    QString fileinvalidimg[4] = {"png","jpg","gif","ico"};
